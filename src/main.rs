...
async fn main() -> anyhow::Result<()> {
    ...
    let traddable_list: serde_json::Value = serde_json
        ::from_str(&data)
        .expect("list.json file was not found");
    ...
    for query in delete_all_queries {
        sqlx::query(query).execute(&app_state.db.to_owned()).await.unwrap();
    }
...
async fn retrieve_and_save_nft(
    ...
    let tasks: Vec<_> = list
        ...
        .map(|character|
            tokio::spawn(
                dump_nft(character, database.clone(), client.clone(), traddable_list.clone())
            )
        )
        .collect();
...
async fn dump_nft(
    ...
    let mut character: Nft = serde_json
        ::from_value(nft_data.clone())
        .expect(&nft_description_error("Fail to get serde_json value from nft", nft_data.clone()));

    let nft_inventory = tokio
        ::spawn(get_nft_inventory(character.transport_id, client.clone())).await
        .expect(&nft_description_error("Fail to get nft inventory", nft_data.clone()))
        .unwrap();
    ...
    let succession = tokio
        ...
        .unwrap();
    let spirits = tokio
        ::spawn(get_nft_spirits(character.transport_id, client.clone())).await
        .expect(&nft_description_error("Fail to get nft spirits", nft_data.clone()))
        .unwrap();
    let magic_orb = tokio
        ::spawn(get_nft_magic_orb(character.transport_id, client.clone())).await
        .expect(&nft_description_error("Fail to get nft magic_orb", nft_data.clone()))
        .unwrap();
    let magic_stone = tokio
        ::spawn(
            get_nft_magic_stone(
                character.transport_id,
                character.class,
        ...
        .expect(&nft_description_error("Fail to get nft magic_stone", nft_data.clone()))
        .unwrap();
    let mystical_piece = tokio
        ::spawn(
        ...
        .expect(&nft_description_error("Fail to get nft mystical_piece", nft_data.clone()))
        .unwrap();

    let (summary, stats, skills, training, buildings, assets, potentials, holy_stuff, codex) =
        tokio::join!(
            tokio::spawn(
                get_nft_summary(
                    character.seq,
    ...
    match (summary, stats, skills, training, buildings, assets, potentials, holy_stuff, codex) {
        (
            Ok(summary),
            Ok(stats),
            Ok(skills),
    ...
    character.inventory_id = db::add_inventory(&pool.clone(), &nft_inventory).await.unwrap();
    ...
    character.magic_stone_id = db::add_magic_stone(pool.clone(), &magic_stone).await.unwrap();
    character.mystical_piece_id = db
        ::add_mystical_piece(pool.clone(), &mystical_piece).await
        .unwrap();

    db::add_nft(&pool.clone().clone(), &character).await.expect(
        &nft_description_error("Fail to add nft to database", nft_data.clone())
    );
